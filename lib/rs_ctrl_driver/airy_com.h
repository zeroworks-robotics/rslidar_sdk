...
class AiryCom : public LidarBase
{
public:
    AiryCom();
    ~AiryCom();
	...
	bool getImuParamsSendData(std::vector<char> & send_data, uint8_t &statusIdx);
	bool getImuParamsResult(ImuParam &getParams);
	bool setImuParamsSendData(ImuParam getParams, std::vector<char> & send_data, uint8_t &statusIdx);

	bool setMotorDir(u8 dir, std::vector<char> & send_data, uint8_t & statusIdx);
	bool getSupplementParamsSendData(std::vector<char> & send_data, uint8_t &statusIdx);
	bool getSupplementParamsResult(NetInfo2 &getParams);
	bool setSomeSupplementParamsSendData(NetInfoSet setParams, std::vector<char> & send_data, uint8_t &statusIdx);
	bool setReflectEnhanceSendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx); // 0:off	 1:on1	2:on2		  3:on3
	bool setGpsBaudSendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx); 

	bool setTrailFilterSendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx);	// [1, 7]
	bool setRainBlockDetectDistanceSendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx); //[0,3]	0: 30cm   1: 20cm	2: 10cm  
	bool setRainDetectSensitivitySendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx);			  // [0, 2]   0: high,	1: middle 2:low 
	bool setBlockDetectSensitivitySendData(u8 level, std::vector<char> & send_data, uint8_t &statusIdx);				 // [0, 2]	 0: high,  1: middle 2:low 

	bool setFixTopRegSendData(std::vector<char> input_data, std::vector<char> & send_data, uint8_t &statusIdx);
	bool setFixBotRegSendData(std::vector<char> input_data, std::vector<char> & send_data, uint8_t &statusIdx);
	bool setFix459RegSendData(std::vector<char> input_data, std::vector<char> & send_data, uint8_t &statusIdx);

	bool getMonitorSendData(std::vector<char> & send_data, uint8_t &statusIdx);
	bool getMonitorResult(float value[64]);
...
private:
	...
	// 包括DataLength--2B, cmd--2B, response--1B, 真实数据data(长度 DataLength - 3 )
    char reg_back_data_[2048];  
